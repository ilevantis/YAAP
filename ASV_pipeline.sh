...
if [[ -s ${5}/${sample}_pear.assembled.fastq ]]; then
...
if [[ -s ${5}/${sample}.trimmed.derep.fasta ]]; then
    ...
    if [[ "fs" -gt 3 ]]; then
        ...
        for i in split_trimmed/*part_* ; do
...
awk -v l="$prefix" '
  BEGIN {cntr = 0}
  /^@[A-Z][0-9]{5}:.{3}:*:*[:space:]*/{cntr++; print l, cntr}
  !/^@[A-Z][0-9]{5}:.{3}:*:*[:space:]*/{print $0}
  ' > ${i%%.fastq}_relabel.fastq
}
...
perc=0
for i in ${outdir}/*3trimmed.fastq ; do
    let perc++
    prog $(( (perc * 100) / n ))
    cat ${i} \
    | seqkit -j ${cpus} seq -m ${min_len} -M ${max_len} \
    > ${i%%3trimmed.fastq}lengthfilter.fastq
done
...
perc=0
for i in ${outdir}/*lengthfilter.fastq ; do
    let perc++
    prog $(( (perc * 100) / n ))
